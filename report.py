...
def createPdf(transaction):
    doc = SimpleDocTemplate("files/contract-%s.pdf" % transaction.id, pagesize=letter)
    
    paragraphs = []
    
    timestamp = config.convertTimestamp(float(transaction.createTimestamp))
    
    paragraphs.append('''<b>THIS LOAN AGREEMENT</b> made as of %s''' % timestamp)
    ...
    paragraphs.append('''<b>%s</b> (the \"Lender\") OF THE SECOND PART''' % transaction.bitcoinAddress)
    paragraphs.append('''   and''')
    ...
    paragraphs.append('''\"Bitcoin Amount\" means the amount of <b>%s</b> bitcoins, to be advanced by the Lender to the Borrower in accordance with the terms of this Agreement.'''% transaction.amount) 
    ...
    paragraphs.append('''Lender: <b>%s</b>''' % transaction.bitcoinAddress) 
    paragraphs.append('''Bitcoin Address: <b>%s</b>''' % transaction.bitcoinAddress)
    paragraphs.append('''GPG Signature: <b>%s</b>''' %  transaction.signature)
