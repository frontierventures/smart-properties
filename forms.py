...
class Save(Resource):
    def __init__(self, newProductCounter, echoFactory):
        self.newProductCounter = newProductCounter
    ...
    def render(self, request):
        ...
        if productId == 0:
            url = '../%s?action=addPhysical' % store.name
        else:
            url = '../%s/%s?action=editDetailsPhysical' % (store.name, productId)

        url = str(url)

        productCategoryId = request.args.get('productCategory')[0]
        productName = request.args.get('productName')[0]
        productDescription = request.args.get('productDescription')[0]
        productStock = request.args.get('productStock')[0]
        productPrice = request.args.get('productPrice')[0]
        note = request.args.get('sellerNote')[0]

        try:
            if request.args.get('isShowcased')[0]:
                isShowcased = True
        except:
            isShowcased = False

        sessionProduct['categoryId'] = productCategoryId
        sessionProduct['name'] = productName
        sessionProduct['description'] = productDescription
        sessionProduct['stock'] = productStock
        sessionProduct['price'] = productPrice
        sessionProduct['note'] = note
        sessionProduct['isShowcased'] = isShowcased

        countries = request.args.get('countries')
        fiat = request.args.get('fiat')

        rates = {}
        for index, countryId in enumerate(countries):
            rates[countryId] = fiat[index]

        sessionProduct['shipping'] = rates

        if error.productName(request, productName):
            return redirectTo(url, request)

        if error.productDescription(request, productDescription):
            return redirectTo(url, request)

        if error.sellerNote(request, note):
            return redirectTo(url, request)

        if not productStock:
            SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.STOCK[0]})
            return redirectTo(url, request)
        else:
            try:
                productStock = int(productStock)
                if productStock < 0:
                    SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.STOCK[1]})
                    return redirectTo(url, request)
            except:
                SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.STOCK[2]})
                return redirectTo(url, request)

        if not productPrice:
            SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.PRICE[0]})
            return redirectTo(url, request)
        else:
            try:
                price = D(productPrice)
            except:
                SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.PRICE[1]})
                return redirectTo(url, request)

            if price <= 0:
                SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.PRICE[2]})
                return redirectTo(url, request)

            #if price.as_tuple().exponent < -8:
            #    SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.EMPTY_PRODUCT_NAME})
            #    return redirectTo(url, request)

        if not countries:
            SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.COUNTRIES[0]})
            return redirectTo(url, request)
        ...
        for index, countryId in enumerate(countries):
            try:
                cost = D(fiat[index])
                rate = (int(countryId), str(cost))
                rates += (rate, )
            except:
                SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.COUNTRIES[1]})
                return redirectTo(url, request)

            #if cost.as_tuple().exponent < -8:
            #    SessionManager(request).setSessionResponse({'class': 1, 'form': 0, 'text': definitions.EMPTY_PRODUCT_NAME})
            #    return redirectTo(url, request)

        if request.args.get('button')[0] == 'Save Product':
            ...
            if productId == 0:
                catalogItems = db.query(CatalogItem).filter(CatalogItem.categoryId == productCategoryId)
                catalogItems = catalogItems.filter(CatalogItem.status != 'deleted')
                catalogItem = catalogItems.order_by(CatalogItem.rank.desc()).first()
                rank = catalogItem.rank + 1
                product = PhysicalProduct(productStatus, timestamp, timestamp, sessionUser['id'],
                                          productName, productDescription, 0, '',
                                          productStock, sessionUser['currencyId'], productPrice)

                catalogItem = CatalogItem(productStatus, 'physical', productCategoryId, rank,
                                          timestamp, timestamp, sessionUser['id'], productName,
                                          0, '', sessionUser['currencyId'], productPrice)

                viewsItem = View(timestamp, 0)

                sellerNote = SellerNote(note)

                product.catalog = [catalogItem]
                product.views = [viewsItem]
                product.sellerNotes = [sellerNote]

                for rate in rates:
                    newShipping = Shipping(product.id, rate[0], rate[1])
                    product.shipping.append(newShipping)
                ...
                catalogItem.imageCount = image['count']

                product.imageHash = image['hash']
                catalogItem.imageHash = image['hash']
                db.commit()

                for connectedProtocol in self.echoFactory.connectedProtocols:
                    connectedProtocol.dataReceived("%s created" % str(product.name))
                    #catalogItem.imageCount = image['count']

            else:
                product = db.query(PhysicalProduct).filter(PhysicalProduct.id == productId).first()
                product.status = productStatus
                product.updateTimestamp = timestamp
                product.name = productName
                product.description = productDescription
                product.stock = productStock
                product.currencyId = sessionUser['currencyId']
                product.price = productPrice

                catalogItem = db.query(CatalogItem).filter(CatalogItem.productId == product.id).first()
                catalogItem.status = productStatus
                catalogItem.categoryId = productCategoryId
                catalogItem.updateTimestamp = timestamp
                catalogItem.name = productName
                catalogItem.currencyId = sessionUser['currencyId']
                catalogItem.price = productPrice

                try:
                    sellerNote = db.query(SellerNote).filter(SellerNote.productId == product.id).first()
                    sellerNote.text = note
                except:
                    newSellerNote = SellerNote(note)
                    product.sellerNotes = [newSellerNote]

                db.query(Shipping).filter(Shipping.productId == product.id).delete()
                for rate in rates:
                    newShipping = Shipping(product.id, rate[0], rate[1])
                    product.shipping.append(newShipping)

                db.commit()
                #image = ProductImage(request, 'update', sessionUser['id'], product.id, product.imageCount).save()
                image = ProductImage(request, product.imageCount, product.imageHash).save()
                product.imageCount = image['count']
                catalogItem.imageCount = image['count']
                ...
                for connectedProtocol in self.echoFactory.connectedProtocols:
                    connectedProtocol.dataReceived("%s updated" % str(product.name))
                    #catalogItem.imageCount = imageCount
